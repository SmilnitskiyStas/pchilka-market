...
SET @has_index = (
  ...
  WHERE table_schema = @schema_name
    ...
    AND index_name = 'idx_product_batches_created_id'
);
SET @sql = IF(
  @has_index = 0,
  'ALTER TABLE product_batches ADD INDEX idx_product_batches_created_id (created_at, id), ALGORITHM=INPLACE, LOCK=NONE',
  'SELECT ''idx_product_batches_created_id already exists'' AS message'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @has_index = (
  ...
  WHERE table_schema = @schema_name
    AND table_name = 'product_batches'
    AND index_name = 'idx_product_batches_store_created_id'
);
SET @sql = IF(
  @has_index = 0,
  'ALTER TABLE product_batches ADD INDEX idx_product_batches_store_created_id (store_id, created_at, id), ALGORITHM=INPLACE, LOCK=NONE',
  'SELECT ''idx_product_batches_store_created_id already exists'' AS message'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @has_index = (
  ...
  WHERE table_schema = @schema_name
    AND table_name = 'product_batches'
    AND index_name = 'idx_product_batches_store_id'
);
SET @sql = IF(
  @has_index = 0,
  'ALTER TABLE product_batches ADD INDEX idx_product_batches_store_id (store_id, id), ALGORITHM=INPLACE, LOCK=NONE',
  'SELECT ''idx_product_batches_store_id already exists'' AS message'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
