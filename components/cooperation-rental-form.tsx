...
const RENTAL_PURPOSE_OPTIONS = [
  'Рекламна стійка / острівець',
  'Промоактивність',
  'Викладка товару в прикасовій зоні',
  'Рекламні матеріали',
  'Інше'
] as const;

type StoredRentalRequest = {
  id: string;
  companyName: string;
...
export default function CooperationRentalForm() {
  ...
  const canSubmit =
    ...
    rentalArea.trim().length > 0 &&
    rentalPurpose.trim().length > 0 &&
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    let uploadedAttachment: StoredRentalRequest['attachment'] = null;
    if (attachment) {
      const uploaded = await uploadRequestAttachment(attachment, {
        folder: 'forms/cooperation/rental'
      });
      uploadedAttachment = {
    ...
    const payload: StoredRentalRequest = {
      ...
      rentalArea: rentalArea.trim().slice(0, 40),
      rentalPurpose,
      rentalPeriod: rentalPeriod.trim().slice(0, 120),
    ...
    try {
      const response = await fetch('/api/requests', {
        ...
        body: JSON.stringify({
          requestType: 'cooperation_rental',
          ...
          sourcePage: typeof window !== 'undefined' ? window.location.pathname : '/cooperation/rental',
          metadata: {
            rentalArea: payload.rentalArea,
            rentalPurpose: payload.rentalPurpose,
      ...
      trackAnalyticsEvent('form_submit', {
        form_name: 'cooperation_rental',
        form_type: 'cooperation',
        page_path: typeof window !== 'undefined' ? window.location.pathname : '/cooperation/rental'
      });
      setSubmitMessage('Дякуємо! Запит на оренду надіслано.');
    } catch (error) {
  ...
  return (
    <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <h2 className="text-xl font-bold text-slate-900 sm:text-2xl">Форма запиту на оренду</h2>
      <p className="mt-1 text-sm text-slate-600">Заповніть заявку, і наш менеджер з оренди зв'яжеться з вами.</p>

      <form onSubmit={handleSubmit} className="mt-5 space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="rental-company" className="block text-sm font-semibold text-slate-900">
          ...
          <div>
            <label htmlFor="rental-contact" className="block text-sm font-semibold text-slate-900">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="rental-phone" className="block text-sm font-semibold text-slate-900">
          ...
          <div>
            <label htmlFor="rental-email" className="block text-sm font-semibold text-slate-900">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="rental-store" className="block text-sm font-semibold text-slate-900">
              Бажаний магазин / адреса <span className="text-red-600">*</span>
            </label>
            <input
              id="rental-store"
              ...
              required
              className="mt-1.5 w-full rounded-xl border border-slate-300 p-3 text-sm text-slate-800 outline-none transition focus:border-brand"
              placeholder="Місто, адреса або перелік магазинів"
            />
          </div>
          <div>
            <label htmlFor="rental-area" className="block text-sm font-semibold text-slate-900">
              Необхідна площа (м2) <span className="text-red-600">*</span>
            </label>
            <input
              id="rental-area"
              value={rentalArea}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="rental-purpose" className="block text-sm font-semibold text-slate-900">
              Ціль оренди <span className="text-red-600">*</span>
            </label>
            <select
              id="rental-purpose"
              value={rentalPurpose}
            ...
            >
              <option value="">Оберіть ціль оренди</option>
              {RENTAL_PURPOSE_OPTIONS.map((item) => (
          ...
          <div>
            <label htmlFor="rental-period" className="block text-sm font-semibold text-slate-900">
              Бажаний період
            </label>
            <input
              id="rental-period"
        ...
        <div>
          <label htmlFor="rental-message" className="block text-sm font-semibold text-slate-900">
          ...
          <textarea
            ...
            required
            className="mt-1.5 w-full rounded-xl border border-slate-300 p-3 text-sm text-slate-800 outline-none transition focus:border-brand"
            placeholder="Опишіть, що саме плануєте розміщувати та додаткові вимоги."
          />
        ...
        <div>
          <label htmlFor="rental-file" className="block text-sm font-semibold text-slate-900">
            Файл (не обов'язково)
          </label>
          <input
            id="rental-file"
