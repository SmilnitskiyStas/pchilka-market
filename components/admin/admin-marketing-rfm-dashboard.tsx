  ...
  return <div>
    ...
    {report ? <><div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[['Покупці', number(report.totals.customers)], ['Чеки', number(report.totals.orders)], ['Оборот', money(report.totals.turnover)], ['Середній чек', money(report.totals.averageCheck)]].map(([label, value]) => <div key={label} className="rounded-2xl border border-slate-200 bg-white p-4"><p className="text-sm text-slate-500">{label}</p><p className="mt-2 text-2xl font-bold text-slate-900">{value}</p></div>)}</div>
      ...
      {detail ? <section className="mt-7 rounded-3xl border border-brand/25 bg-white p-5 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand">Деталізація сегмента</p><h2 className="mt-1 text-2xl font-bold text-slate-900">{detail.segment.label}</h2><p className="mt-1 text-sm text-slate-600">{detail.segment.description}</p></div><button type="button" onClick={() => setSelected(null)} className="rounded-xl border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700">Закрити</button></div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[['Чеків', number(detail.behavior.orders)], ['Чеків / покупця', detail.behavior.ordersPerCustomer.toFixed(1)], ['Сер. давність', `${Math.round(detail.behavior.averageRecencyDays)} дн.`], ['Середній LTV', money(detail.behavior.averageLifetimeValue)], ['Останній візит', detail.behavior.latestVisit ?? '—'], ['Піковий день', detail.behavior.busiestWeekday ?? '—'], ['Пікова година', detail.behavior.busiestHour ?? '—'], ['Сумарний LTV', money(detail.behavior.totalLifetimeValue)]].map(([label, value]) => <div key={label} className="rounded-2xl bg-slate-50 p-3"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 font-bold text-slate-900">{value}</p></div>)}</div>
        <div className="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)]"><div><h3 className="text-lg font-bold text-slate-900">Топ товарів за охопленням</h3><div className="mt-3 overflow-x-auto rounded-2xl border border-slate-200"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-3 py-2">Товар</th><th className="px-3 py-2 text-right">Охоплення</th><th className="px-3 py-2 text-right">Покупці</th><th className="px-3 py-2 text-right">Чеки</th></tr></thead><tbody>{detail.topProducts.map((product) => <tr key={`${product.code}-${product.name}`} className="border-t border-slate-100"><td className="px-3 py-2"><p className="font-medium text-slate-900">{product.name}</p><p className="text-xs text-slate-500">{product.barcode ?? 'Штрихкод не вказано'}</p></td><td className="px-3 py-2 text-right">{product.reach.toFixed(1)}%</td><td className="px-3 py-2 text-right">{number(product.customers)}</td><td className="px-3 py-2 text-right">{number(product.orders)}</td></tr>)}</tbody></table></div></div><div><h3 className="text-lg font-bold text-slate-900">Поведінка покупців</h3><div className="mt-3 rounded-2xl bg-slate-50 p-4"><p className="text-sm font-semibold text-slate-800">Розподіл за днями</p><div className="mt-3 space-y-2">{detail.behavior.weekdayDistribution.map((item) => <div key={item.label} className="flex items-center gap-2 text-xs"><span className="w-6 text-slate-600">{item.label}</span><span className="h-2 flex-1 overflow-hidden rounded-full bg-slate-200"><span className="block h-full rounded-full bg-brand" style={{ width: `${item.share}%` }} /></span><span className="w-10 text-right text-slate-600">{item.share.toFixed(1)}%</span></div>)}</div><p className="mt-4 text-sm font-semibold text-slate-800">Топ-3 години</p><p className="mt-1 text-sm text-slate-600">{detail.behavior.topHours.map((item) => `${item.label} (${item.share.toFixed(1)}%)`).join(' · ') || '—'}</p></div></div></div>
