...
async function walkCatalogs(directory: string, relativeDirectory = ''): Promise<CatalogFile[]> {
  ...
  const catalogs = await Promise.all(
    entries.map(async (entry) => {
      ...
      if (entry.isDirectory()) return walkCatalogs(absolutePath, relativePath);
...
async function listCatalogs(): Promise<CatalogFile[]> {
  ...
  const catalogs = await walkCatalogs(directory);
