...
const ids: RfmSegmentId[] = ['champions', 'loyal', 'potential_loyalists', 'new_customers', 'at_risk', 'need_attention', 'about_to_sleep', 'lost'];
...
function segmentFor(row: CustomerRow): RfmSegmentId { const r = +row.r_score, f = +row.f_score, m = +row.m_score; if (r >= 4 && f >= 4 && m >= 4) return 'champions'; if (r >= 3 && f >= 3) return 'loyal'; if (r >= 4 && f >= 2) return 'potential_loyalists'; if (r >= 4 && f === 1) return 'new_customers'; if (r <= 2 && f >= 3 && m >= 3) return 'at_risk'; if (r <= 3 && f >= 2) return 'need_attention'; if (r <= 2 && f === 1) return 'about_to_sleep'; return 'lost'; }

async function customers(client: Client, from: string, to: string) {
  const key = `${from}:${to}`, hit = cache.get(key); if (hit && hit.expiresAt > Date.now()) return hit.rows;
  const result = await client.query<CustomerRow>(`WITH metrics AS (SELECT code_client::text AS customer_id, COUNT(*)::bigint AS orders, SUM(COALESCE(sum_order,0))::numeric AS turnover, (CURRENT_DATE-MAX(date_order)::date)::int AS recency_days, MAX(date_order)::date::text AS latest_visit FROM pos.order_client WHERE date_order >= $1::date AND date_order < ($2::date + interval '1 day') AND code_client IS NOT NULL AND code_client <> 0 AND COALESCE(sum_order,0)>0 GROUP BY code_client) SELECT customer_id,orders::text,turnover::text,recency_days::text,latest_visit,NTILE(5) OVER (ORDER BY recency_days DESC)::text AS r_score,NTILE(5) OVER (ORDER BY orders ASC)::text AS f_score,NTILE(5) OVER (ORDER BY turnover ASC)::text AS m_score FROM metrics`, [from, to]);
